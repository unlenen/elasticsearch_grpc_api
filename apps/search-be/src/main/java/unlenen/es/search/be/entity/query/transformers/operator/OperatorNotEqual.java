 ...
 */
public class OperatorNotEqual extends AbstractOperator {

    @Override
    public Query createStatementQuery(Statement statement, boolean insideCondition) {
        if (insideCondition) {
            return QueryBuilders.term(
                    fn -> fn.field(statement.getField())
                            .value(fieldValue -> getValue(statement.getValue(), fieldValue)));
        } else {
            return QueryBuilders.bool(
                    b -> b.mustNot(mn -> mn.match(m -> m.field(statement.getField()).query(statement.getValue()))));
        }
    }
